...
namespace Map
{
    ...
    public class Patch
    {
        ...
        // Take an odd numbered patch, and slice it into several patches
        public void GenerateQuilt(List<Patch> patches)
        {
            // Do not allow even numbers through
            if (Columns % 2 == 0 || Rows % 2 == 0)
                throw new Exception("Cannot Generate Quilt from even dimensioned patches");

            for (int c = 0; c < Columns - 2; c += 2)
            {
                for (int r = 0; r < Rows - 2; r += 2)
                {
                    Patch patch = new Patch(LiteralMesh) { Columns = 3, Rows = 3 };
                    
                    patch.FlatVertices.Add(FlatVertices[GetIndex(c, r)]);
                    patch.FlatVertices.Add(FlatVertices[GetIndex(c + 1, r)]);
                    patch.FlatVertices.Add(FlatVertices[GetIndex(c + 2, r)]);

                    patch.FlatVertices.Add(FlatVertices[GetIndex(c, r + 1)]);
                    patch.FlatVertices.Add(FlatVertices[GetIndex(c + 1, r + 1)]);
                    patch.FlatVertices.Add(FlatVertices[GetIndex(c + 2, r + 1)]);

                    patch.FlatVertices.Add(FlatVertices[GetIndex(c, r + 2)]);
                    patch.FlatVertices.Add(FlatVertices[GetIndex(c + 1, r + 2)]);
                    patch.FlatVertices.Add(FlatVertices[GetIndex(c + 2, r + 2)]);
        ...
        public void BuildIndices()
        {
            ...
            for (int y = 0; y < height; ++y)
            {
                for (int x = 0; x < width; ++x)
                {
                    ...
                    index.Add((short)start);
                    index.Add((short)(start + 1));
                    index.Add((short)(start + Columns));
                    index.Add((short)(start + 1));
                    index.Add((short)(start + 1 + Columns));
                    index.Add((short)(start + Columns));
                }
            }
